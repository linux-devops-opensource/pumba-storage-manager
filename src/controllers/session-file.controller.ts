...
const BASE_URL = process.env.S3_BASE_URL ? process.env.S3_BASE_URL : 'DEFAULT_S3_BASE_URL variable not defined';
const PREFIX = process.env.BUCKET_PREFIX ? process.env.BUCKET_PREFIX : 'BUCKET_PREFIX variable not defined';
...
export class SessionFileController {
	...
	async create(
		...
		await parser(request, async (filed: any) => {
			const token = process.env.S3_TOKEN ? process.env.S3_TOKEN : 's3 token not definde';
			await this.s3Service.uploadFile(
				id,
				filed.fieldContent.fileName,
				filed.fieldContent.fileStream,
				filed.fieldContent.fileType,
				token
			);
			await this.s3Service.publicAccess(id, filed.fieldContent.fileName, token);
			file = {
	...
	async patch(
		...
		file: Partial<File>,
		@param.query.object('where', getWhereSchemaFor(File))
		where?: Where<File>
	): Promise<Count> {
	...
	async delete(
		...
		for (const file of files) {
			await this.s3Service.deleteFile(
				id,
				file.name,
				process.env.S3_TOKEN ? process.env.S3_TOKEN : 'S3_TOKEN is not defined'
			);
		}
